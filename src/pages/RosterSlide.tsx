...
export const RosterSlide = () => (
  <div className="h-full flex flex-col relative overflow-hidden">
    ...
    <div className="flex-1 p-3 md:p-6 relative z-10 flex flex-col justify-center gap-4 max-w-[1400px] mx-auto w-full overflow-y-auto md:overflow-hidden">
        ...
        {CAST_PRIMARY.map((char, i) => (
          <div key={char.id} className="group relative flex flex-col bg-coh-panel/60 border border-coh-border overflow-hidden hover:border-coh-orange/50 transition-all duration-500">
            <div className="h-32 md:h-48 relative overflow-hidden">
              ...
              {/* Hover drawer — slides up from bottom (hidden on mobile) */}
              <div className="hidden md:block absolute inset-x-0 bottom-0 translate-y-full group-hover:translate-y-0 transition-transform duration-500 ease-out bg-coh-bg/95 backdrop-blur-sm border-t border-coh-orange/40 p-3">
                <p className="text-coh-gray text-[10px] leading-relaxed">{char.desc}</p>
              </div>
            </div>
            <div className="p-2 md:p-3 flex flex-col flex-1">
              <div className="font-mono text-[7px] md:text-[8px] text-coh-orange tracking-widest mb-0.5 uppercase">{char.codename}</div>
              <div className="font-bebas text-xl md:text-3xl text-white tracking-widest leading-none mb-0.5">{char.name}</div>
              <div className="font-mono text-[7px] md:text-[8px] text-coh-gray tracking-widest uppercase mb-1">{char.role}</div>
              {/* Mobile-only truncated description */}
              <p className="md:hidden text-coh-gray text-[9px] leading-relaxed line-clamp-2 mb-1">{char.desc}</p>
              <div className="mt-auto flex justify-between items-center">
                <div className="font-mono text-[7px] md:text-[8px] text-white tracking-widest">{char.actor}</div>
                <div className={`font-mono text-[6px] md:text-[7px] px-1.5 py-0.5 border tracking-widest uppercase
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-2 md:gap-3">
        {CAST_SUPPORTING.map((char) => (
          <div key={char.id} className="group relative flex bg-coh-panel/60 border border-coh-border overflow-hidden hover:border-coh-orange/50 transition-all duration-500">
            <div className="w-16 md:w-20 shrink-0 relative overflow-hidden">
            ...
            <div className="p-2 md:p-2.5 flex flex-col flex-1 min-w-0 relative">
              <div className="font-bebas text-lg md:text-xl text-white tracking-widest leading-none mb-0.5">{char.name}</div>
              <div className="font-mono text-[6px] md:text-[7px] text-coh-orange tracking-widest uppercase mb-1">{char.role}</div>
              {/* Mobile-only truncated description */}
              <p className="md:hidden text-coh-gray text-[8px] leading-relaxed line-clamp-2 mb-1">{char.desc}</p>
              <div className="mt-auto flex justify-between items-center gap-1">
                <div className="font-mono text-[6px] md:text-[7px] text-white tracking-widest truncate">{char.actor}</div>
                <div className={`font-mono text-[5px] md:text-[6px] px-1 py-0.5 border tracking-widest uppercase shrink-0
                  ${char.status === 'ATTACHED' ? 'text-green-500 border-green-500/30 bg-green-500/10' :
              ...
              {/* Hover overlay for supporting cast (hidden on mobile) */}
              <div className="hidden md:flex absolute inset-0 bg-coh-bg/95 backdrop-blur-sm p-2.5 opacity-0 group-hover:opacity-100 transition-opacity duration-400 flex-col justify-center">
